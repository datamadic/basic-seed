...
module.exports = React.createClass({
        ...
        render: function() {
            ...
            return <div className = "ls-window" > < div className = {
                nonShrinkingClasses
            } > < div className = "window-control" > < i onClick = {
                this.minApp
            }
            className = "fa fa-minus min" > < /i>
            <i onClick={this.maxApp} className="fa fa-square-o"></i > < i onClick = {
                this.closeApp
            }
            className = "fa fa-times" > < /i>
        </div > < div className = "id-bar" > < div className = "ticker" > < div className = "symbol" > {
                this.state.symbol
            } < /div>
                <div className="status">
                    <div className={indicatorClasses}></div > < /div>
            </div > < div className = "name" > {
                this.state.name
            } < /div>
        </div > < div className = "dashboard" > < div className = "last" > < div className = "price" > {
                Number(this.state.last_price || 0).toFixed(2)
            } < /div>
                <div className="volume">
                    {Number(this.state.last_qty || 0).toString().replace(/\
            B( ? = (\d {
                3
            }) + ( ? !\d)) / g, ",")
    } < /div>
                <div className="expander">
                    <i onClick={this.toggleExpand} className={iconClasses}></i > < /div>
                
            </div > < div className = "time" > < span className = "timeval" > {
        this.state.last
    } < /span>
                <div className="timebottom"></div > < /div>
            <div className="detail">
                <div className="label">REF</div > < div className = "val" > {
        this.state.reference_price
    } < /div>
                <div className="label">DEPTH</div > < div className = "val" > {
        this.state.detailDepthBuy
    } | {
        this.state.detailDepthSell
    } < /div>
                <div className="expander"></div > < /div>
        </div > < /div>
        
    <div className="grid">
        <i className={connectIcoClasses}></i > < div className = {
        gridClasses
    } > < Grid gridHeight = {
